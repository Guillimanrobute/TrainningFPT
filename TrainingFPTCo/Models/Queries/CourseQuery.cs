...
namespace TrainingFPTCo.Models.Queries
{
    public class CourseQuery
    {
        ...
        public List<CourseDetail> GetAllDataCourses(string? searchString, string status)
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                ...
                if (!string.IsNullOrEmpty(searchString) || !string.IsNullOrEmpty(status))
                {
                    ...
                    {
                        sql += $" AND ([co].Name LIKE '%{searchString}%' OR [co].Description LIKE '%{searchString}%' OR [ca].Name LIKE '%{searchString}%')";
                    }
...
)
        {
            ...
            WHERE [Id] = @Id";
                ...
                cmd.Parameters.AddWithValue("@Image", imageCourse);
                cmd.Parameters.AddWithValue("@StartDate", startDate);
                cmd.Parameters.AddWithValue("@EndDate", endDate != null ? (object)endDate.Value : DBNull.Value);
                cmd.Parameters.AddWithValue("@Status", status);
